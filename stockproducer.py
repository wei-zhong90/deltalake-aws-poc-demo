...
class StockProvider(BaseProvider):
    ...
    def produce_msg(self):
        ...
        message = {
            'order_id': self.order_id(stockname),
            'order_owner': stockname,
            'order_value': self.stock_value(stockname),
            'timestamp': int(ts*1000)
        }
        key = {'order_id': self.order_id(stockname)}
        return message, key